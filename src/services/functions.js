import { separator, bucklingSafety, tankData, motorData, VPipe, pipesData, pipesSData, coolerVZ, coolerData } from "./data";

const isString = variable => typeof variable === "string";

export const round = (num, param=10) => isString(num) ? num : Math.round(num * param) / param;
...
export const Qback = item => pumpCounting(item).Qback;
 export const Qmax = (project)=> project.map(({unit}) => unit.map(item => Qback(item))).flat().reduce((a,b)=>a+b);
...
export const KITtitle = (order) => {
    const { Size, type } = order.tank?.title ? order.tank?.tankData : { Size:'__', type: '__' };
    const Pkeys = Object.keys(order).filter(key => key.includes('motor'));
    const P = Pkeys.length ? Pkeys.map(key => order[key]?.motorData?.power).join('/') : '___';
    const Qkeys = Object.keys(order).filter(key => key.includes('pump'));
    const Q = Qkeys.length ? Qkeys.map(key => round(getQ(order[key]?.pumpData?.CC, order[key]?.pumpData?.n))).join('/') : '___';
