...
<template>
    <path v-if="spool[1] === 'A' || ['A', 'C', 'D', 'D1'].some(el => el === spool)"
        :d="`M${x} ${y}h${-1.5 * h} v${h} h${h} v${-h} v${h} h${h} v${-h}z`" stroke="black" stroke-width="1"
        fill="white" />
    <path v-if="spool[1] === 'B' || ['B', 'Y', 'Y1'].some(el => el === spool)"
        :d="`M${x} ${y}h${-0.5 * h} v${h} h${h} v${-h} v${h} h${h} v${-h}z`" stroke="black" stroke-width="1"
        fill="white" />
    ...
    <ValveX v-if="spool === 'D' || spool === 'C'" :x="x - h" :y="y" :h="h" />
    <ValveII v-if="spool === 'D' || spool === 'C'" :x="x" :y="y" :h="h" />
