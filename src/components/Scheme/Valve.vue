...
<template>
  ...
  <path :d="`M${x() + sl() / 2 - (0.5 - 0.46) * sl()} ${y()} v${h() / 6}`" stroke="black" stroke-width="2"
    fill="none" />
  <path :d="`M${x() + sl() / 2 + (0.5 - 0.46) * sl()} ${y()} v${h() / 6}`" stroke="black" stroke-width="2"
    fill="none" />
  ...
  <Valve3 v-if="
    data.spool.length === 1 &&
    ['A', 'B', 'C', 'D', 'Y'].every((el) => el !== data.spool)" :x="x() + sl() / 2" :y="y() + 10" :h="h()"
    :spool="data.spool" />
  <Valve2 v-else :x="x() + sl() / 2" :y="y() + 10" :h="h()" :spool="data.spool" />
  <text :x="x() + sl() / 2" :y="y() + h() * 2" font-family="Arial" :font-size="sl() / 15" fill="black"
    text-anchor="middle">
    {{ data.spool }}
  </text>
</template>
