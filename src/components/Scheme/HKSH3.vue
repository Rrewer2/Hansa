...
<template>
  ...
  <template v-else-if="data.form === '⇐'">
    <path
      :d="`M${x - 0.25 * sl} ${y + sh} v${-sh / 8} h${sl / 2 + sh / 5} v${-sh + sh / 8} h${-sh / 5 - sh / 5} v${-sh / 4} v${sh / 2}`"
      stroke="black"
      stroke-width="2"
      fill="none"
    />
    <path
      :d="`M${x + 0.25 * sl} ${y + sh} h${-sl / 2 - sh / 5} v${-sh} h${sh / 2.5} v${-sh / 4} v${sh / 2}`"
      stroke="black"
      stroke-width="2"
      fill="none"
    />
  ...
  <text :x="x" :y="y + 36 / 3" font-family="Arial" :font-size="36" text-anchor="middle">... {{ data.z }}</text>
</template>
