...
const { x, y, unit, R, xT, xM, yM, c, order } = defineProps(["x", "y", "unit", "R", "xT", "xM", "yM", "c", "order"]);
...
const yValve = ({ throttle, check, directPress }) => (!!throttle + !!check + !!directPress) * getSh(unit);
const yHKHMP = (data) => (!!data.throttle + !!data.check) * getSh(unit);
const yHKHQ = (data) => !!data.check * getSh(unit);
const yUp = () => y + 300 - getSh(unit) / 8 - (3 * getSh(unit)) / 2;
const yVFC = () => yUp();
const yVNR = (data) => yUp() - getSh(unit) * !!data.flowControl;
const xHKSH = (unit, i) => x(c) + getSL1(unit) + (i + unit.start) * (getSL(unit) * 1.1) + getSL(unit) / 2;
const yHKSH = (data) => yUp() - getSh(unit) * (!!data.flowControl + !!data.nonReturn);
</script>
