...
<template>
  <template v-if="data.check">
    ...
    <path v-if="!HKHR[data.check]?.A" :d="`M${x + 0.46 * sl} ${y} v${sh}`" stroke="black" stroke-width="2"
      fill="none" />
    <path v-if="!HKHR[data.check]?.B" :d="`M${x + (1 - 0.46) * sl} ${y} v${sh}`" stroke="black" stroke-width="2"
      fill="none" />
    <template v-if="HKHR[data.check]?.A">
      <path :d="`M${x + 0.46 * sl} ${y} v${sh / 4} h${-sh / 4} v${sh / 2} h${sh / 4} v${sh / 4}`" stroke="black"
        stroke-width="2" fill="none" />
      <path v-if="!HKHR[data.check]?.B" :d="`M${x + (1 - 0.46) * sl} ${y + sh / 2 + sh / 12} h${-sh / 2}`"
        stroke="black" stroke-width="2" stroke-dasharray="5 5" fill="none" />
      <Point v-if="!HKHR[data.check]?.B" :x="x + (1 - 0.46) * sl" :y="y + sh / 2 + sh / 12" />
      <path v-if="HKHR[data.check]?.B" :d="`M${x + 0.46 * sl} ${y + sh / 2 + sh / 4} v${-sh / 8} h${sh / 2}`"
        stroke="black" stroke-width="2" stroke-dasharray="5 5" fill="none" />
      <Point v-if="HKHR[data.check]?.B" :x="x + 0.46 * sl" :y="y + sh / 2 + sh / 4" />
    ...
    <template v-if="HKHR[data.check]?.B">
      <path :d="`M${x + (1 - 0.46) * sl} ${y} v${sh / 4} h${sh / 4} v${sh / 2} h${-sh / 4} v${sh / 4}`" stroke="black"
        stroke-width="2" fill="none" />
      <path v-if="!HKHR[data.check]?.A" :d="`M${x + 0.46 * sl} ${y + sh / 2 + sh / 12} `" stroke="black"
        stroke-width="2" stroke-dasharray="5 5" fill="none" />
      <path v-if="HKHR[data.check]?.A"
        :d="`M${x + (1 - 0.46) * sl} ${y + sh / 2 + sh / 4} v${-sh / 16} h${-sh / 2.5} v${-sh / 16}`" stroke="black"
        stroke-width="2" stroke-dasharray="5 5" fill="none" />
      <Point v-if="HKHR[data.check]?.A" :x="x + (1 - 0.46) * sl" :y="y + sh / 2 + sh / 4" />
      <CheckValve :x="x + (1 - 0.46) * sl + sh / 4" :y="y + sh / 2" :r="r" deg="0" />
    </template>
