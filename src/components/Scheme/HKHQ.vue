  ...
  <template v-if="data.throttle">
    ...
    <template v-if="HKHQ[data.throttle]?.left">
      ...
      <CheckValve
        ...
        :y="y + sh / 2"
        :r="r"
        :deg="HKHQ[data.throttle]?.deg"
      />
      <Point :x="x + 0.46 * sl - sh / 4" :y="y + sh / 4" />
      <Point :x="x + 0.46 * sl - sh / 4" :y="y + (3 * sh) / 4" />
      <path
        :d="`M${x + 0.46 * sl - sh / 4} ${y + sh / 4} h${-sh / 2} v${sh / 2} h${sh / 2}`"
        stroke="black"
        stroke-width="2"
        fill="none"
      />
    ...
    <template v-if="HKHQ[data.throttle]?.right">
      ...
      <path
        :d="`M${x + (1 - 0.46) * sl + sh / 4} ${y + sh / 4} h${sh / 2} v${sh / 2} h${-sh / 2}`"
        stroke="black"
        stroke-width="2"
        fill="none"
      />
