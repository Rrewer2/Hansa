...
<template>
  ...
  <text v-for="(_, n) in N + start" :x="x + sl1 + sl * 0.35 + sl * 1.1 * n" :y="y - sh / 10" font-family="Arial"
    :font-size="20" text-anchor="middle">A{{ n + 1 }}</text>
  <text v-for="(_, n) in N + start" :x="x + sl1 + sl * 0.65 + sl * 1.1 * n" :y="y - sh / 10" font-family="Arial"
