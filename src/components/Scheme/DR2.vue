...
<template>
  ...
  <DR2nextSection v-for="(_, n) in N" :sl="sl" :sh="sh" :x="x + sl1 + sl * 1.1 * (n + start)" :y="y" :r="sh / 16" />
  ...
  <text
    ...
    >B{{ n + 1 }}</text
  >
  <text v-if="text && type" :x="x - sh / 3" :y="y + sh / 10 + sh / 2" font-family="Arial" :font-size="30" text-anchor="middle">
    {{ text }} bar
  </text>
</template>
