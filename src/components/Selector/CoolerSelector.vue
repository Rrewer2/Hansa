...
<template>
  <article v-if="meta.cooler">
    <h2>
      Chłodnica<span> {{ order.cooler?.title }}</span>
    </h2>
    ...
    <table>
      ...
      <tbody>
        <tr
          v-for="{ title, ...elem } in filteredCooler(project, cooler)?.title
        ...
        >
          <td class="tal">
            <input
              type="radio"
              :id="title"
              v-model="order.cooler"
              :value="{ title, coolerData: elem }"
              name="title"
