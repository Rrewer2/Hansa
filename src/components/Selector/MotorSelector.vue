<script setup>
import { ref } from "vue";
import { engineMountData, enginesData, freqData, motorData, motorSizes } from "../../services/data";
import { getMaxPower, reducedPower, round, setPressure } from "../../services/functions";
...
const filteredMotors = () =>
  enginesData.filter(
    ({ power, mount }) =>
      power * 1.25 >= reducedPower(powerUNIT.unit) && power * 0.75 <= reducedPower(powerUNIT.unit) && isOKWithMount(mount),
  );
const motorsByPower = () => enginesData.filter(({ power, mount }) => power === project[i].P && (!powerUNIT.mount || isOKWithMount(mount)));
const motorsBySize = () => enginesData.filter(({ size, mount }) => size === motorSize.value && (!powerUNIT.mount || isOKWithMount(mount)));
const motors = () => (project[i].P ? motorsByPower() : motorSize.value ? motorsBySize() : filteredMotors());
const filteredMotorData = () => {
  if (order[`pump${i}`]?.title) {
    return motorData.filter(
      (el) => el <= getMaxPower({ VFU: order[`pump${i}`]?.pumpData.CC, n: powerUNIT.n, p: order[`pump${i}`]?.pumpData.pmax }),
    );
  }
  return motorData;
};
...
<template>
  <SmthSelector v-bind="{ meta, order }" Name="motor" :index="i" :logic="motors">
    <InputItem data="n">
