...
<template>
  <article>
    <h2 :class="open && 'bgc-g'">
      Zbiornik<span> {{ order.tank?.title }}</span>
    </h2>
    ...
    <table>
      ...
      <tbody>
        <tr
          v-for="{ title, ...elem } in getStandartTank(
            meta,
        ...
        >
          <td class="tal">
            <input
              type="radio"
              :id="title"
              :value="{ title, tankData: elem }"
              name="elem"
              v-model="order.tank"
              :checked="title === order.tank?.title"
              class="mx"
            />
            <a
