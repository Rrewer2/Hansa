...
const ventilationFilter = () =>
  order["pump" + 0]?.pumpData?.out?.startsWith("Bore")
    ? []
    : ventilation.filter(({ title, q, filterGrade, thread }) => q >= 10 * Qmax(project));
const getXvrT = () => {
...
<template>
  <SmthSelector v-if="Qmax(project)" v-bind="{ meta, order }" Name="filter" index="" :logic="filteredFilter" :after="getXvrT" />
  <SmthSelector v-if="Qmax(project)" v-bind="{ meta, order }" Name="ventilation" index="" :logic="ventilationFilter" />
</template>
