...
const filteredCoupling = () => {
  const data = order[`motor${i}`]?.motorData?.mount.at(-1) === "4"
      ? couplingsKitData
      : couplingData;
...
<template>
  <SmthSelector v-bind="{ project, meta, order }" Name="bellhousing" :index="i" :logic="filteredBellhousing" />
  <SmthSelector v-bind="{ project, meta, order }" Name="coupling" :index="i" :logic="filteredCoupling" />
</template>
