...
const controlElemLength = (project) => {
    ...
    return (project.flatMap(powerUnit => powerUnit.unit.map((unit) =>
        getSL1(unit) + getSL(unit) * unit.HKSH.length
    )));
};
const xControl = (project) => controlElemLength(project).reduce((acc, curr) => [...acc, (acc.length ? acc[acc.length - 1] : 0) + curr + 300], [300]);
...
<template>
    <svg xmlns="http://www.w3.org/2000/svg" :viewBox="`0 0 ${screenSize * 2} ${screenSize}`" class="">
        <ControlUnit v-for="unit, c in project.flatMap(({ unit, mount }) => unit.map(el => ({ ...el, mount })))"
        ...
        <PumpUnit v-for="pumps, i in project" :x="tankGap + 200 + 1000 * i" :y="yTank() - 1.25 * R()"
            :pumps="pumps.unit" :mount="pumps.mount" :R="R()">{{ pumps.n }}</PumpUnit>
