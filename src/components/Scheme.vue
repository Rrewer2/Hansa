...
<template>
  <svg
    ...
    :viewBox="`0 0 ${screenSize * 2} ${screenSize}`"
    class=""
  >
    <ControlUnit
      v-for="(unit, c) in project.flatMap(({ unit, mount }) =>
        unit.map((el) => ({ ...el, mount })),
      )"
      :x="(c) => controlGap(project) + xControl(project)[c]"
      :y="100"
      v-bind="{ unit, c }"
      :R="R()"
      :xT="xFilter"
    ...
    <Cooler
      v-if="meta.cooler"
      :x="xFilter"
      :y="meta.cooler === 2 ? yTank() - 180 : yTank() - 50"
      :a="R() / 1.5"
    />
    <Filter
      :x="xFilter"
      :y="meta.cooler === 1 ? yTank() - 180 : yTank()"
      :a="R()"
    />
  </svg>
</template>
