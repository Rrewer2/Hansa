    ...
    <div class="flex-row">
      ...
      <div v-for="(_, ind) in rest" class="flex-col">
        <InputItem :data="ind">
          ...
          <select v-if="ind === 'mount'" v-model="project[k].mount" :disabled="order[`motor${k}`]?.title" :id="ind" class="w-75">
          ...
          <select
            v-if="ind === 'P'"
            v-model="project[k].P"
            @change="() => setPressure(unit, project[k].P)"
            :disabled="order[`motor${k}`]?.title"
            :id="ind"
          >
